test_that("run_wf_amplicon builds the default Nextflow command", {
  ...
  expect_equal(res$env, c("NXF_SYNTAX_PARSER=v1", "NXF_ANSI_LOG=false"))
...
test_that("run_wf_amplicon can override Nextflow environment", {
  ...
  expect_equal(res2$env, c("NXF_ANSI_LOG=false", "NXF_SYNTAX_PARSER=v2"))
})

test_that("run_wf_amplicon can reduce Nextflow console output", {
  res <- run_wf_amplicon(quiet = TRUE, dry_run = TRUE, echo = FALSE)

  expect_equal(res$args[1:3], c("-q", "run", "julibeg/wf-amplicon"))
...
test_that("run_wf_amplicon validates numeric arguments", {
  ...
  expect_error(
    run_wf_amplicon(quiet = "yes", dry_run = TRUE, echo = FALSE),
    "quiet"
  )
})
