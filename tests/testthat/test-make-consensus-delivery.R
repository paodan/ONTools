...
test_that("make_consensus_delivery skips downstream steps when consensus is missing", {
  proj <- tempfile("ont-project-")
  delivery_dir <- tempfile("amplicon-delivery-")
  run_root <- file.path(
    proj,
  ...
  fastq_dir <- file.path(run_root, "fastq_pass_trim")
  ...
  dir.create(file.path(run_root, "results", "wf_amplicon_denovo", "PROJECT001_1600"), recursive = TRUE)
  ...
  expect_warning(
    ...
    "Consensus FASTA was not generated"
  )

  expect_equal(res$trim$PROJECT001_1600$status, "skipped_no_consensus")
  expect_equal(res$igv$PROJECT001_1600$status, "skipped_no_consensus")
  expect_equal(res$delivery$PROJECT001_1600$status, "skipped_no_consensus")
  expect_false(dir.exists(file.path(delivery_dir, "PROJECT001_1600", "consensus_results")))
})

test_that("make_consensus_delivery does not require consensus when downstream steps are disabled", {
  proj <- tempfile("ont-project-")
  delivery_dir <- tempfile("amplicon-delivery-")
  run_root <- file.path(
    proj,
