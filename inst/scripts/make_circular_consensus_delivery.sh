...
Usage:
  bash make_circular_consensus_delivery.sh \
    ...
    [--variants-vcf /path/to/variants.vcf.gz] \
    [--variants-vcf-index /path/to/variants.vcf.gz.csi] \
    [--variants-af /path/to/variants.af.tsv] \
    [--variants-af-filtered /path/to/variants_gt0.05.af.tsv] \
...
Outputs:
  <output>/<project>_consensus_delivery/
    ...
    03_qc/
      consensus_stats.tsv
      variants.vcf.gz, if provided
      variants.vcf.gz.csi, if provided
      variants.af.tsv, if provided
      variants_gt0.05.af.tsv, if provided
    04_md5/
      md5.txt
    manifest.tsv
    README.txt
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --variants-af)
      ...
      shift 2
      ;;
    --variants-af-filtered)
      VARIANTS_AF_FILTERED="${2:-}"
      shift 2
      ;;
    --assembly-info)
      ASSEMBLY_INFO="${2:-}"
...
[[ -n "$VARIANTS_VCF" ]] && check_file "$VARIANTS_VCF" "Variants VCF"
[[ -n "$VARIANTS_VCF_INDEX" ]] && check_file "$VARIANTS_VCF_INDEX" "Variants VCF index"
[[ -n "$VARIANTS_AF" ]] && check_file "$VARIANTS_AF" "Variants AF table"
[[ -n "$VARIANTS_AF_FILTERED" ]] && check_file "$VARIANTS_AF_FILTERED" "Filtered variants AF table"
...
if [[ -n "$VARIANTS_VCF" ]]; then
  log "Copying variant VCF."
  copy_file "$VARIANTS_VCF" "$DELIVERY_DIR/03_qc/variants.vcf.gz" "variants_vcf"
fi

if [[ -n "$VARIANTS_VCF_INDEX" ]]; then
  log "Copying variant VCF index."
  copy_file "$VARIANTS_VCF_INDEX" "$DELIVERY_DIR/03_qc/variants.vcf.gz.csi" "variants_vcf_index"
fi

if [[ -n "$VARIANTS_AF" ]]; then
  log "Copying variant allele-frequency table."
  copy_file "$VARIANTS_AF" "$DELIVERY_DIR/03_qc/variants.af.tsv" "variants_af"
fi

if [[ -n "$VARIANTS_AF_FILTERED" ]]; then
  log "Copying filtered variant allele-frequency table."
  copy_file "$VARIANTS_AF_FILTERED" "$DELIVERY_DIR/03_qc/variants_gt0.05.af.tsv" "variants_af_filtered"
fi
...
(
  cd "$DELIVERY_DIR"
  seqkit stats -a -T 01_consensus/consensus.fasta > 03_qc/consensus_stats.tsv
)
printf '%s\t%s\t%s\n' \
  "consensus_stats" \
...
Directory contents:
  ...
  03_qc/            Basic consensus FASTA statistics and optional variant QC files.
...
Key files:
  ...
  03_qc/variants.vcf.gz
      Variant calls generated from reads aligned back to the final consensus, if provided.
  03_qc/variants.af.tsv
      Variant allele-frequency table, if provided.
  03_qc/variants_gt0.05.af.tsv
      Filtered allele-frequency table, for example ALT_AF > 0.05, if provided.
  manifest.tsv
...
目录内容：
  ...
  03_qc/            共识序列 FASTA 基础统计和可选变异频率 QC 文件。
...
关键文件：
  ...
  03_qc/variants.vcf.gz
      reads 回贴最终共识序列后生成的变异位点 VCF（如提供）。
  03_qc/variants.af.tsv
      变异位点等位基因频率表（如提供）。
  03_qc/variants_gt0.05.af.tsv
      过滤后的变异频率表，例如 ALT_AF > 0.05（如提供）。
  manifest.tsv
      交付包内文件列表及其来源路径。
  04_md5/md5.txt
      文件完整性校验值，可用于传输后核对。
